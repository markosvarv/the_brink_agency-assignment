...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`pages_blocks_hero\` (
  	...
  	\`supporting_text\` text DEFAULT 'High quality industrial services and emergency maintenance available 24/7 across all regions.',
  	\`cta_label\` text DEFAULT 'Contact us',
  	\`cta_link\` text DEFAULT '/contact',
  	\`secondary_cta_label\` text DEFAULT 'Contact Us',
  ...
  await db.run(sql`CREATE TABLE \`header\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`top_bar_show_top_bar\` integer DEFAULT true,
  	\`top_bar_text\` text DEFAULT 'Emergency number – available 24/7',
  	\`top_bar_phone\` text DEFAULT '0900 – 11 22 333',
  	\`logo_text\` text DEFAULT 'THE BRINK' NOT NULL,
  	\`logo_image_id\` integer,
  	\`show_search\` integer DEFAULT true,
  	\`cta_button_label\` text DEFAULT 'Contact us',
  ...
  `)
  await db.run(sql`CREATE INDEX \`header_logo_image_idx\` ON \`header\` (\`logo_image_id\`);`)
}
